...
namespace RimionshipServer.Users
{
    public class UserManager : UserManager<RimionUser>
    {
        ...
        public async Task<List<string>> GetInactivePlayerIds(TimeSpan interval)
        {
            var inactiveUserTimeOffset = DateTimeOffset.Now.Subtract(interval);
            return await Users
                 .Include(u => u.LatestStats)
                 .Where(u => u.LatestStats != null)
                 .Where(u => u.LatestStats!.Timestamp < inactiveUserTimeOffset)
                 .Select(u => u.Id)
                 .ToListAsync();
        }
    }
